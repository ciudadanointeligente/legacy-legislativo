 ...
 */
class Parlamentario extends BaseParlamentario {

	public function __toString() {
		return sprintf('%s %s (%s)', $this->getNombre(), $this->getApellidos(), ($this->getActivo() == 1) ? 'activo' : 'inactivo');
	}

	public function getVotacionesSala() {
		$this->getSenadorDiputado() == 'D' ? $camara = 'C.Diputados' : $camara = 'Senado';
		return Doctrine_Core::getTable('Votacion')->createQuery('v')->where('v.camara = ? AND v.en_sala = ? AND v.visible = ?', array($camara, 1, 1))->groupBy('v.id_proyecto_ley')->orderBy('v.fecha DESC, v.tipo ASC')->execute();
	}

	public function getVotacionesComision() {
		$this->getSenadorDiputado() == 'D' ? $camara = 'C.Diputados' : $camara = 'Senado';
		return Doctrine_Core::getTable('VotacionComision')->createQuery('v')->where('v.camara = ?', $camara)->groupBy('v.id_debate')->execute();
	}

	public function getVotacionesComisionDebate($id_debate) {
		$this->getSenadorDiputado() == 'D' ? $camara = 'C.Diputados' : $camara = 'Senado';
		return Doctrine_Core::getTable('VotacionComision')->createQuery('v')->where('v.camara = ?', $camara)->andWhere('v.id_debate = ?', $id_debate)->execute();
	}

	public function getPartidos() {
		return Doctrine_Core::getTable('Partido')->createQuery('p')->orderBy('p.sigla')->execute();
	}

	public function getComunas() {
		return Doctrine_Core::getTable('Comuna')->createQuery('c')->orderBy('c.nombre')->execute();
	}

	public function getMociones() {
		$leyes = null;
		$autores = Doctrine_Core::getTable('Autor')->createQuery('a')->where('a.id_parlamentario = ?', $this->getIdParlamentario())->execute();
		if (count($autores) > 0) {
			foreach ($autores as $i => $autor)
				if (count($autor->getAutorProyectoLey()) > 0) {
					...
					if ($leyes)
						$leyes->merge($autor->getAutorProyectoLey());
					else
						$leyes = $autor->getAutorProyectoLey();
				}
		}
