...
$fechas_html = str_get_html(file_get_contents_curl($fechas_url,$hoy,$hoy));

$num=0;
$inicio="HORA INICIO: ".date("H:i:s")."\n";
foreach($fechas_html->find('td[class="TEXTpais"]') as $nro)
{
    ...
    if($o->selectQ()==0)
    {
        ...
        unset($autores_html);
        $num++;
        $msj.="* Proyecto de Ley $nro_boletin--> Acaba de agregarse exitosamente en 'Area de Administración'.";
    }
    else
    {
        $msj.="* Proyecto de Ley $nro_boletin --> No se agregó en este momento, este ya se encontraba disponible en 'Area de Administración'.";
    }
    $msj.="\n";
}
//echo $msj;
$fin="HORA TERMINO: ".date("H:i:s")."\n\n";
$archivo = "log/extraeProyectoLey_".date("Y-m-d").".txt";
$fp = fopen($archivo, "a+");
$string = $inicio.$fin.$msj.="\n\n\n\n";
