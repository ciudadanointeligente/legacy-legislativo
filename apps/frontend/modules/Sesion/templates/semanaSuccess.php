...
<?php if (is_numeric($semana)): ?>

  <?php slot('breadbrumb') ?>
    <li><a href="<?php echo url_for('Sesion/index') ?>">Análisis legislativo</a></li>
    <li class="actual">Otros proyectos de la tabla</li>
  <?php end_slot() ?>
      ...
      <?php $offset=(date("w")-1); if ($offset<0) $offset=6; if ($offset>=5) $offset-=7; $dias=array("Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"); ?>
        ...
        <?php $lunes = date("m/d/Y", mktime(0,0,0,date('m'), date('d')-$offset-$semana*7, date('Y') )); $viernes = date("m/d/Y", mktime(0,0,0,date('m'), date('d')-$offset+4-$semana*7, date('Y') )) ?>
      <div class="session">
              ...
              <?php if (strtotime($sesion->getFecha()) >= strtotime($lunes) && strtotime($sesion->getFecha()) <= strtotime($viernes)): ?>
                <?php foreach ($sesion->getProyectoLey() as $proyecto): ?>
                  <?php if (count($proyecto->getDebates($proyecto->getIdProyectoLey()))==0): ?>
                    <?php //if (!isset($shown)): ?><tr><td><?php echo $dias[$sesion->getDateTimeObject('fecha')->format('w')].', '.$sesion->getDateTimeObject('fecha')->format('d/m').' - '.$sesion->getCamara(); $shown=true; ?></td></tr><?php //endif; ?>
                    ...
                    <!--<tr class="mas"><td><img src="/images/proyectos_mas.png"></td></tr>-->
                  <?php endif; ?>
                <?php endforeach; ?>
              <?php endif; ?>
            <?php endforeach; ?>
...
<?php elseif ($semana=='proxima'): ?>
  
  <?php //semanas regionales y distritales
  ...
  <?php slot('breadbrumb') ?>
    <li class="actual">Análisis legislativo de la próxima semana</li>
  <?php end_slot() ?>

  <img src="/images/banner_proyectos.png"><br>
  <div id="sessiones">

    <div class="session_scroll">
    
      <?php $offset=(date("w")-1); if ($offset<0) $offset=6; $offset-=7; $dias=array("Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"); ?>
      <?php for ($i=0; $i<4; $i++): ?>
